...
import com.example.corp.base.adapter.ListAdapterItem
...
data class Person( override val id: Int, val fullName: String) : ListAdapterItem {
    override fun toString(): String {
        return StringBuilder(fullName).append(" (").append(id.toString()).append(") ").toString()
    }
}
...
class DataSource @Inject constructor() {
    ...
    suspend fun fetch(next: String?) : DataSourceResponse{
        val processResult = processRequest(next)

        delay((processResult.waitTime * 1000).toLong())
        return  DataSourceResponse(processResult.fetchResponse, processResult.fetchError)
    }
